...
def find_download_metadata():
    ...
    logger.debug(f"Looking for SD downloads in download folder: {download_path}")
    manifest_files = []
        ...
        if manifest_file.is_file():
            try:
                with open(manifest_file, 'r') as f:
                    ...
                    logger.debug(f"Manifest file contents: {manifest}")
...
def write_info_file(upload_folder, camera_info, file_manifest):
    ...
    try:
        ...
        logger.debug(f"Writing info file: {info_file_path}")
